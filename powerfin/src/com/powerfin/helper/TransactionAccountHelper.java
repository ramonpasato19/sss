...
public class TransactionAccountHelper {
	...
	public static TransactionAccount createTransactionAccount(Account account,
		...
		return TransactionAccountHelper.createTransactionAccount(account, subaccount, category, debitOrCredit, value, BigDecimal.ZERO, null, transaction, updateBalance, officialValue, dueDate, null);
	}

	
	public static TransactionAccount createCustomCreditTransactionAccount(Account account,
			BigDecimal value, BigDecimal quantity, Unity unity, Transaction transaction, Category category, Branch branch) throws Exception {
		return TransactionAccountHelper.createCustomCreditTransactionAccount(account, value, quantity, unity, transaction, category, null, branch);
	}
	
	public static TransactionAccount createCustomDebitTransactionAccount(Account account,
			BigDecimal value,BigDecimal quantity, Unity unity, Transaction transaction, Category category, Branch branch) throws Exception {
		return TransactionAccountHelper.createCustomDebitTransactionAccount(account, value,quantity,unity, transaction,  category, null, branch);
	}
	
	public static TransactionAccount createCustomCreditTransactionAccount(Account account,
			BigDecimal value, BigDecimal quantity, Unity unity, Transaction transaction, Category category, Types.YesNoIntegerType updateBalance, Branch branch) throws Exception {
		return TransactionAccountHelper.createTransactionAccount(account, 0, category, DebitOrCredit.CREDIT, value, quantity, unity, transaction, updateBalance, null, null, branch);
	}
	
	public static TransactionAccount createCustomDebitTransactionAccount(Account account,
			BigDecimal value, BigDecimal quantity, Unity unity, Transaction transaction, Category category, Types.YesNoIntegerType updateBalance, Branch branch) throws Exception {
		return TransactionAccountHelper.createTransactionAccount(account, 0, category, DebitOrCredit.DEBIT, value, quantity, unity, transaction, updateBalance, null, null, branch);
	}
	
	public static TransactionAccount createTransactionAccount(Account account,
			Integer subaccount, Category category, DebitOrCredit debitOrCredit,
			BigDecimal value, BigDecimal quantity, Unity unity, Transaction transaction, Types.YesNoIntegerType updateBalance,
			Types.YesNoIntegerType officialValue,
			Date dueDate,
		...
		if (account.getBranch() == null)
			throw new InternalException("the_account_does_not_have_a_branch", account);
		...
		ta.setBranch(branch==null?account.getBranch():branch);
	...
	public static TransactionAccount createTransactionAccountForTransferItem(Account account,
			Category category, DebitOrCredit debitOrCredit,
			BigDecimal value, BigDecimal quantity, Transaction transaction,
			Branch branch) throws Exception{
		...
		if (branch == null)
			throw new InternalException("branch_is_required_for_transaction_account", account, category, value);
		...
		if (quantity != null && quantity.compareTo(BigDecimal.ZERO)<0)
			throw new InternalException("incorrect_quantity_for_transaction_account", quantity);
		...
		ta.setValue(value);
		ta.setQuantity(quantity);
		ta.setUnity(transaction.getOrigenUnity());
		ta.setTransaction(transaction);
