...
public class PayableSalePortfolioSaveAction extends ViewBaseAction  {
	...
	public void execute() throws Exception {
		...
		if (subAction.equals("COLLECT"))
		{
			...
			List<AccountSoldPaytable> accountPaytables = XPersistence.getManager().createQuery("SELECT pt FROM AccountSoldPaytable pt "
			...
			int count = 1;
			for (AccountSoldPaytable pt : accountPaytables)
			{
		...
		}else
		{
			...
			TransactionModule transactionModule = XPersistence.getManager().find(TransactionModule.class, BatchProcessHelper.PAYABLE_SALE_PORTFOLIO_TRANSACTION_MODULE);
			...
			for (BatchProcessDetail detail : batchProcessDetails)
			{
				...
				AccountSoldPaytable quota = accountPaytables.get(0);
