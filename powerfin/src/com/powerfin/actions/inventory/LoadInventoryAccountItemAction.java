...
public class LoadInventoryAccountItemAction extends ViewBaseAction {
	...
	private String getNaviteQueryKardex(String schema) {
		...
		sql.append("			AND A.ACCOUNT_STATUS_ID NOT IN ('004','007') ");
		...
		sql.append("        	AI.ACCOUNT_ID = AID.ACCOUNT_INVOICE_ID ");
		sql.append("        	AND AI.ACCOUNT_ID = A.ACCOUNT_ID ");
		sql.append("        	AND A.PRODUCT_ID = '105' ");
		sql.append("			AND A.ACCOUNT_STATUS_ID NOT IN ('004','007') ");
		sql.append("        	AND AID.ACCOUNT_DETAIL_ID = :PACCOUNT_ITEM_ID "); 
		sql.append("        	AND TO_DATE(TO_CHAR(AI.ISSUE_DATE, 'YYYY/MM/DD'), 'YYYY/MM/DD') BETWEEN  :PFROM_DATE AND :PTO_DATE ");        	
		sql.append("        	AND A.BRANCH_ID  IN (SELECT B.BRANCH_ID  FROM "+schema+".BRANCH B WHERE B.NAME LIKE :PBRANCH_NAME) ");
		...
		sql.append("	        AI.ACCOUNT_ID = AID.ACCOUNT_INVOICE_ID ");
		sql.append("	        AND AI.ACCOUNT_ID = A.ACCOUNT_ID ");
		sql.append("	        AND A.PRODUCT_ID ='1064' ");
		sql.append("			AND A.ACCOUNT_STATUS_ID NOT IN ('004','007') ");
		sql.append("	        AND AID.ACCOUNT_DETAIL_ID = :PACCOUNT_ITEM_ID "); 
		sql.append("	        AND TO_DATE(TO_CHAR(AI.ISSUE_DATE, 'YYYY/MM/DD'), 'YYYY/MM/DD') BETWEEN :PFROM_DATE AND :PTO_DATE ");
		sql.append("			AND A.BRANCH_ID IN (SELECT B.BRANCH_ID  FROM "+schema+".BRANCH B WHERE B.NAME LIKE :PBRANCH_NAME) ");
		sql.append("        UNION  ");
		sql.append("        SELECT  ");
		sql.append("	        T.TRANSACTION_ID || '_' || :PACCOUNT_ITEM_ID AS ACCOUNT_ID_AUX,  ");
		sql.append("	        T.ACCOUNTING_DATE AS ISSUE_DATE, ");
		sql.append("	        T.REMARK AS REMARK_AUX,  ");
		sql.append("	        SUM(0) AS UNIT_COST, ");
		sql.append("        	TA.ACCOUNT_ID AS ACCOUNT_DETAIL_ID,  ");
		sql.append("        	SUM(TA.QUANTITY) AS INCOME,  ");
		sql.append("        	SUM(0) AS EXPENSES ,  ");
		sql.append("        	SUM(TA.QUANTITY) AS BALANCE, ");
		sql.append("	        TA.BRANCH_ID AS BRANCH_ID_AUX  ");
		sql.append("		FROM  ");
		sql.append("        	"+schema+".ACCOUNT A, ");
		sql.append("        	"+schema+".TRANSACTION T, ");
		...
		sql.append("	   	GROUP BY ACCOUNT_ID_AUX, ISSUE_DATE,REMARK_AUX,ACCOUNT_DETAIL_ID, BRANCH_ID_AUX  ");
		sql.append("        UNION  ");
		sql.append("        SELECT  ");
		sql.append("	        T.TRANSACTION_ID || '_' || :PACCOUNT_ITEM_ID AS ACCOUNT_ID_AUX,  ");
		sql.append("	        T.ACCOUNTING_DATE AS ISSUE_DATE, ");
		...
		sql.append("	        SUM(0) AS UNIT_COST, ");
		sql.append("        	TA.ACCOUNT_ID AS ACCOUNT_DETAIL_ID,  ");
		sql.append("        	SUM(0) AS INCOME,  ");
		sql.append("        	SUM(TA.QUANTITY) AS EXPENSES ,  ");
		sql.append("        	SUM(0-TA.QUANTITY) AS BALANCE, ");
		sql.append("	        TA.BRANCH_ID AS BRANCH_ID_AUX  ");
		sql.append("		FROM  ");
		sql.append("	        "+schema+".ACCOUNT A, ");
		sql.append("	        "+schema+".TRANSACTION T, ");
