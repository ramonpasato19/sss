...
<div class="phone-frame-header"> 
	...
	<xava:link action='<%=subview.getNewCollectionElementAction()%>' argv='<%="viewObject="+viewName%>'>
		<div class="phone-frame-action">
			<% MetaAction newAction = MetaControllers.getMetaAction(subview.getNewCollectionElementAction()); %>					
			<p><%=newAction.getLabel()%></p>			
		</div>
	</xava:link>
	<xava:link action='<%=subview.getRemoveSelectedCollectionElementsAction()%>' argv='<%="viewObject="+viewName%>'>
		<div class="phone-frame-action">
			...
			MetaAction removeAction = MetaControllers.getMetaAction(subview.getRemoveCollectionElementAction()); 
			%>					
			<p><%=removeAction.getLabel()%></p>			
		</div>
	</xava:link>
	<% } %>
